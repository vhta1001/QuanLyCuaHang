...
namespace BLL
{
    public class ExcelExport_HDB
    {
        ...
        private void BuildReplacerCurrentDate(ref Dictionary<string, string> pReplacer,string count,string tongtien ,string ngaylap,string idhd,string tennv,string tenKH,DateTime pngaygiao)
        {
            if (pReplacer != null)
            {
                DateTime currentDate = DateTime.Now;
                string ngay = "Ngày " + currentDate.Day + " tháng " + currentDate.Month + " năm " + currentDate.Year;
                string ngaygiao = "Ngày " + pngaygiao.Day + " tháng " + pngaygiao.Month + " năm " + pngaygiao.Year;
                pReplacer.Add("%NgayThangNam", ngay);
                pReplacer.Add("%So",idhd);
                pReplacer.Add("%count", count);
                pReplacer.Add("%TongTien",tongtien );
                pReplacer.Add("%KhachHang", tenKH);
                pReplacer.Add("%Ngay", ngaylap);
                pReplacer.Add("%TenNV", tennv);
                pReplacer.Add("%Giao", ngaygiao);
            }
        }
        ...
        private MemoryStream GetTemplateStream(string viewName)
        {
            ...
            switch (viewName)
            {
                ...
                case T_BieuMau:
                    ...
                    arrByte = File.ReadAllBytes("HoaDonBan.xls").ToArray();
                    break;
        ...
        public bool ExportKhoa(List<Class1> dataSource, ref string fileName, bool isPrintPreview, string count, string tongtien, string ngaylap, string idhd, string tennv, string tenKH,DateTime pngaygiao)
        {
            // Check if data is null
        ...
        //    replacer.Add(V_TONGSO, "100");
            ...
            BuildReplacerCurrentDate(ref replacer,count,tongtien,ngaylap,idhd,tennv,tenKH,pngaygiao);
